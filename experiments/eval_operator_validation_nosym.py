...
if __name__ == "__main__":
    ...
    kernel.load_state_dict(torch.load("checkpoints/kernel_nosym.pt"))
    kernel.eval()
...
# Diagnostic: symmetry violation magnitude
...
x_i = x.view(100, 1).expand(100, 100).reshape(-1)
x_j = x.view(1, 100).expand(100, 100).reshape(-1)

with torch.no_grad():
    G_ij = kernel(x_i, x_j).view(100, 100)
    G_ji = kernel(x_j, x_i).view(100, 100)

sym_error = torch.norm(G_ij - G_ji) / torch.norm(G_ij)
print("Relative symmetry violation:", sym_error.item())
